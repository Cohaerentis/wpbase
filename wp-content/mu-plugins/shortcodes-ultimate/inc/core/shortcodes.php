...
class Su_Shortcodes {
    ...
    public static function gmap( $atts = null, $content = null ) {
	    ...
	    $url .= '?q=' . urlencode( su_scattr( $atts['address'] ) );
	    $url .= '&amp;output=embed';
	    $url .= '&amp;ie=UTF8';
	    $url .= '&amp;oe=UTF8';
	    $url .= '&amp;z=' . urlencode( $atts['zoom'] );
	    $url .= '&amp;f=' . urlencode( $atts['display'] );
	    $url .= '&amp;t=' . urlencode( $atts['type'] );
	    if (!empty($layer)) $url .= '&amp;layer=' . urlencode( $atts['layer'] );
	    if (!empty($print)) $url .= '&amp;pw=1';
	    if (!empty($expinfo)) $url .= '&amp;iwd=1';
	    $url .= '&amp;view=' . urlencode( $atts['view'] );
	    $url .= '&amp;iwloc=' . urlencode( $atts['info'] );
	    $url .= '&amp;hl=' . urlencode( $atts['lang'] );

	    $html  = '<div class="su-gmap su-responsive-media-' . $atts['responsive'] . su_ecssc( $atts ) . '">';
	    $html .= '<iframe width="' . $atts['width'] . '" height="' . $atts['height'] . '" src="' . $url . '"></iframe>';
	    $html .= '</div>';
        return $html;
    }

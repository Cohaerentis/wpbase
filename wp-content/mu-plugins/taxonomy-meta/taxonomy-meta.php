...
class RW_Taxonomy_Meta {
   ...
   function show( $tag, $taxonomy ) {
      ...
      foreach ( $this->_fields as $field ) {
         ...
         // AEA - Escape HTML attributes, except for wysiwyg type
   ...
   function show_field_select( $field, $meta ) {
      ...
      // AEA - START - Allow to use a callback function to get options
      $options = $field['options'];
      if (is_string($options) && is_callable($options)) {
         $options = call_user_func($options, $field, $meta);
      }
      ...
      // AEA - END - Allow to use a callback function to get options
      echo "</select>";
      $this->show_field_end( $field, $meta );
   }
      ...
      */
      ...
      foreach ( $this->_fields as $field ) {
         ...
         if ( method_exists( $this, $save_func ) ) {
            // AEA - Use call_user_func_array function to call by reference
            call_user_func_array( array( $this, 'save_field_' . $type ),
                                  array( &$meta, $field, $old, $new ) );
         } else {
   ...
   function save_field_wysiwyg( &$meta, $field, $old, $new ) {
      ...
      // AEA - Disable wpautop: $new = wpautop( $new );
      $this->save_field( $meta, $field, $old, $new );
   }
